...
class TokenWordRule[A, B]() extends Rule[A, B] {
  override def apply(ruleLookup: RuleLookup[A, B], cursor: Cursor[A], assembler: Assembler[A, B]): MatchResult[A, B] = {
    ...
    val result: MatchResult[A, B] = if (cursor.isEnd) MatchFail(ruleName, s"expected $ruleName but got end of input")
    else cursor.value match {
      case x: TokenWord => MatchSuccess(ruleName, cursor, cursor.next, assembler.push(cursor.value))
